...
namespace :audit do
  ...
  task :symbol_report, %i[symbol start_date end_date] => :environment do |_t, args|
  ...
  task :daily_summary, [:date] => :environment do |_t, args|
    ...
    # Alert conditions
    alerts = []
    alerts << '⚠️  ALERT: No data ingestion runs today!' if ingestion_runs.none?
    if total_decisions.positive? && (failed.to_f / total_decisions) > 0.3
      alerts << "⚠️  ALERT: High failure rate (#{(failed.to_f / total_decisions * 100).round}%)"
    end
  ...
  task :failure_analysis, %i[start_date end_date strategy] => :environment do |_t, args|
