...
class SyncCommitteeMembershipsFromGithub < GLCommand::Callable
  ...
  def call
    ...
    return unless unmatched_list.any?
  ...
  def fetch_yaml(filename)
    ...
    raise "Failed to download #{filename}: #{response.code} #{response.message}" unless response.is_a?(Net::HTTPSuccess)
  ...
  def find_committee_by_code(github_code)
    ...
    return unless github_code.length > 4
