# Audit Trail for Trading System

> **Detailed Technical Documentation**: See `/docs/architecture/trade-outbox-pattern/`
...
**Detailed technical specifications** are available in `/docs/architecture/trade-outbox-pattern/`:
- `data-ingestion-logging.md` - DataIngestionRun, junction tables, GLCommand implementation
- `api-payload-storage.md` - STI pattern, ApiPayload models, retention policy
- `trade-decision-model.md` - Hybrid normalization, state machine, GLCommand
- `trade-execution-model.md` - Synchronous execution, error handling

**Database Schema**: 5 new tables
- `data_ingestion_runs` - Audit log for rake tasks
- `data_ingestion_run_records` - Junction table (polymorphic)
- `api_payloads` - STI table for all API request/response pairs
- `api_call_logs` - Links payloads to ingestion runs
- `trade_decisions` - Outbox pattern for trade intent
- `trade_executions` - Execution results with FK to payloads

**Pack Structure**: `packs/audit_trail/`
- Models: DataIngestionRun, DataIngestionRunRecord, ApiPayload (STI), ApiCallLog, TradeDecision, TradeExecution
- Commands: LogDataIngestion, CreateTradeDecision, ExecuteTradeDecision
- Queries: SymbolActivityReport, StrategyPerformanceReport, FailureAnalysisReport

**Dependencies**:
- GLCommand gem (already in use)
- AASM gem (for state machines)
- PostgreSQL 16 (JSONB + GIN indexes)
...
- Symbol activity report: <200ms for full year (with proper indexes)

**Storage Estimate**: ~26MB/year for aggressive trading (250 days × 20 decisions/day)

**Testing Strategy**: 89 tasks across 6 phases (see `tasks.md`)
